...
(deftest translate-a-point
  (testing "When we want to translate a point"
    (let [trans (transform/translate 5 -3 2)
      ...
      (is (tuple= (m/mmul trans p) (tuple/point 2 1 7))
...
(deftest scaling
  (testing "When scaling a point"
    (let [trans (transform/scale 2 3 4)
      ...
      (is (tuple= (m/mmul trans p) (tuple/point -8 18 32))
  ...
  (testing "When shrinking a point we can use the inverse"
    (let [inverse-trans (m/inverse (transform/scale 2 3 4))
      ...
      (is (tuple= (m/mmul inverse-trans p) (tuple/point -2 2 2))
  ...
  (testing "When reflecting a point we can use negative values"
    (let [trans (m/inverse (transform/scale -1 1 1))
      ...
      (is (tuple= (m/mmul trans p) (tuple/point 4 6 8))
          "x-component flips")))
  ...
  (testing "When scaling a vector"
    (let [trans (transform/scale 2 3 4)
      ...
      (is (tuple= (m/mmul trans v) (tuple/vector -8 18 32))
...
(deftest rotating
  (testing "When rotating around the x-axis"
    (let [original-p (tuple/point 0 1 0)
      ...
      (is (tuple= expected (m/mmul rot-45-deg original-p))
          "Rotate by 45-deg")

      (is (tuple= original-p (m/mmul (m/inverse rot-45-deg) expected))
          "Rotate back by 45-deg")

      (is (tuple= (tuple/point 0 0 1) (m/mmul rot-90-deg original-p))
  ...
  (testing "When rotating around the y-axis"
    (let [original-p (tuple/point 0 0 1)
      ...
      (is (tuple= (tuple/point 1 0 0) (m/mmul rot-90-deg original-p))
...
(deftest shearing
  (let [x-by-y (transform/shearing 1 0 0 0 0 0)
    ...
    (testing "Moving components in proportion to each other"
      (is (tuple= (tuple/point 5 3 4) (m/mmul x-by-y p))
          "Multiple of y added back to x")

      (is (tuple= (tuple/point 6 3 4) (m/mmul x-by-z p))
          "Multiple of z added back to x")

      (is (tuple= (tuple/point 2 5 4) (m/mmul y-by-x p))
          "Multiple of x added back to y")

      (is (tuple= (tuple/point 2 7 4) (m/mmul y-by-z p))
          "Multiple of z added back to y")

      (is (tuple= (tuple/point 2 3 6) (m/mmul z-by-x p))
          "Multiple of x added back to z")

      (is (tuple= (tuple/point 2 3 7) (m/mmul z-by-y p))
  ...
  (let [p (tuple/point 1 0 1)
    ...
    (testing "Transformations can be applied in sequence."
      (let [p_A (m/mmul A p)
        ...
        (is (tuple= (tuple/point 1 -1 0) p_A)
            "A multiplied by p")

        (is (tuple= (tuple/point 5 -5 0) p_BA)
            "B multiplied by p_A")

        (is (tuple= (tuple/point 15 0 7) p_CBA)
            "C multiplied by p_BA")))

    (testing "Transformations can be composed and applied."
      (is (tuple= (tuple/point 15 0 7) (m/mmul C B A p))))))
